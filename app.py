...
BLOCKED_PATTERNS = {
    "discriminacao": [
        # Cadastre aqui termos que sua equipe decidir bloquear.
        #
        # Esses termos devem ficar somente no backend,
        # nunca mostrados na interface.
...
# Limite simples por conexão.
# Quando houver autenticação definitiva,
# o ideal é utilizar user_id.
...
def normalize_role(
    ...
    """
    Aceita os valores utilizados tanto pelo
    AstroLearn quanto pelo chatbot.
    """
...
def safe_short_text(
    ...
    """
    Normaliza textos curtos de contexto.
    """
...
def build_system_instructions(
    ...
    """
    Cria as instruções da IA conforme
    o perfil da sessão.
    """
...
def build_welcome_message(
    ...
    if (
        ...
        return (
            f"{greeting} "
            "Eu sou o **Sparky**. "
            "Pronto para construir, programar "
            "e aprender? 🚀"
        )
    ...
    return (
        f"{greeting} "
        "Eu sou o **Sparky**. "
        "Posso ajudar com aulas, projetos maker "
        "e atividades para sua turma."
    )
...
def update_profile(
    ...
    Neste estágio, esses dados vêm do frontend
    principal do AstroLearn.
    """
...
def create_chat(
    ...
    """
    Cria uma conversa independente
    para a sessão atual.
    """
...
def get_chat(
    ...
    if (
        ...
        app.logger.info(
            (
                "Criando conversa para sessão %s "
                "| modelo=%s | perfil=%s"
            ),
...
def generate_response(
    ...
    """
    Gera resposta utilizando o modelo principal
    e tenta modelos alternativos em falhas temporárias.
    """
    ...
    for (
        ...
        try:
            ...
            if (
                not has_fallback
                or
                not is_transient_provider_error(
                    error
                )
            ):
                raise
...
def is_rate_limited(
    ...
    """
    Proteção simples contra muitas mensagens
...
def handle_connect(
    ...
    """
    Inicia a sessão usando exclusivamente
    o perfil recebido da plataforma.

    Não existe seleção manual de aluno/professor
    dentro do chatbot.

    IMPORTANTE:
    neste estágio o perfil ainda vem do frontend.
    A versão final deve validar JWT do AstroLearn.
    """
    ...
    if (
        ...
        app.logger.warning(
            (
                "Conexão recusada sem "
                "contexto de usuário: %s"
            ),
            request.sid,
        )
...
# -----------------------------------------------------------------------------
# Troca manual de perfil bloqueada
# -----------------------------------------------------------------------------
...
def handle_set_profile(
    ...
    emit_error(
        (
            "O perfil do Sparky é definido "
            "automaticamente pela conta autenticada "
            "no AstroLearn e não pode ser alterado "
            "no chatbot."
        ),

        code=
            "profile_managed_by_platform",
    )
...
def handle_send_message(
    ...
    # -------------------------------------------------------------------------
    # Filtro de conteúdo da mensagem
    # -------------------------------------------------------------------------
    ...
    if blocked:
        ...
        emit_error(
            (
                ...
                "não permitido nesta plataforma educacional."
            ),
    ...
    app.logger.info(
        (
            "Mensagem recebida da sessão %s "
            "(%d caracteres) | request=%s"
        ),
    ...
    try:
        ...
        if (
            ...
            emit_error(
                (
                    "O Sparky não pode apresentar "
                    "esse tipo de conteúdo nesta "
        ...
        app.logger.exception(
            (
                "Falha ao responder à sessão %s: %s"
            ),
...
def handle_reset_conversation():
    """
    Limpa o histórico da IA,
    mas mantém o perfil da conta.
    """
